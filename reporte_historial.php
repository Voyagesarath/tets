...
$this->Text(65,14,'Departamento de Comunicación y difusión',0,'C', 0);
$this->Ln(30);
}
...
$this->Cell(100,10,'Editorial',0,0,'L');
}
}
...
$strConsulta = "SELECT * from user where id_user =  '$user'";
$user = mysql_query($strConsulta);
...
$pdf->Cell(0,6,'Nombre: '.$fila['nombre_user'],0,1);
$pdf->Cell(0,6,'apaterno: '.$fila['apaterno'].' '.$fila['amaterno'].' '.$fila['amaterno'],0,1);
$pdf->Cell(0,6,'Departamento: '.$fila['departamento'],0,1);
$pdf->Cell(0,6,'coordinador: '.$fila['coordinador'],0,1);
$pdf->Ln(10);
...
$pdf->Row(array('FECHA', 'nombre_user', 'num_copias', 'descripcion'));
...
$strServicop = "SELECT servicop.fecha_consulta, servicop.num_copias, servicop.diagnostico, user.nombre_user, user.apaterno
FROM servicop
Inner Join user ON servicop.id_user = user.id_user
Inner Join servicios ON servicop.id_serv = medicos.id_serv
WHERE user.id_user = '$paciente'";
// ejecutamos la consulta
$historial = mysql_query($strServicop);
// listamos la tabla de historial de visitas de cada paciente
$numfilas = mysql_num_rows($historial);
for ($i=0; $iRow(array($fila['fecha_consulta'], $fila['num_copias'], $fila['nombre_user'], $fila['descripcion']));
}
//La ultima linea
